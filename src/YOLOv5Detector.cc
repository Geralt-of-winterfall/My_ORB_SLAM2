...
void YOLOv5Detector::init(std::string onnxpath)
{
    ...
    std::string file="/home/robertkidder/DeepSORT/build//coco_80_labels_list.txt";
...
void YOLOv5Detector::detect(cv::Mat & frame, std::vector<detect_result> &results)
{
    ...
    cv::Mat image;
	image = cv::Mat::zeros(cv::Size(_max, _max), CV_8UC3);
	cv::Rect roi(0, 0, w, h);
    image(roi);
	frame.copyTo(image);
    ...
    cv::Mat det_output(preds.size[1], preds.size[2], CV_32F, preds.ptr<float>());//单通道
