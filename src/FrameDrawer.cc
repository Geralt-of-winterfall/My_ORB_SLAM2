    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(vbVO[i] || vbMap[i])
            {
                ...
                //框出检测物体
                DrawDetections(im, mResults,mClasses);
                mResults.clear();
                mClasses.clear();
            }
        }
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    mResults = pTracker->mvResults;
    mClasses = pTracker->mvClasses;
...
//*
//画出框选物体
void FrameDrawer::DrawDetections(cv::Mat & frame, std::vector<detect_result> &results, std::vector<std::string> &classes)
{
    for(auto dr : results)
    {
